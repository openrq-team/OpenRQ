...
/**
 */
final class PaddedByteArrayEncoder {

    static PaddedByteArrayEncoder newArray(byte[] array, int paddedLen) {

        return newArray(array, 0, array.length, paddedLen);
    }

    static PaddedByteArrayEncoder newArray(byte[] array, int off, int len, int paddedLen) {

        ArrayUtils.checkArrayBounds(off, len, array.length);
        if (paddedLen < 0) throw new IllegalArgumentException("negative padded length");

        return new PaddedByteArrayEncoder(array, off, len, paddedLen);
    }
    ...
    private PaddedByteArrayEncoder(byte[] array, int off, int len, int paddedLen) {
