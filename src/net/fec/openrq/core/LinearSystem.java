...
final class LinearSystem {
    ...
    private static byte[][] generateGAMMA(int Kprime, int S)
    {
        ...
        for (int row = 0; row < Kprime + S; row++)
        {
            ...
            {
                if (row >= col) GAMMA[row][col] = OctectOps.getExp((row - col) % 256);
                else continue;
            }
        }
    ...
    public static byte[] enc(int Kprime, byte[][] C, Tuple tuple, int T)
    {
        ...
        {
            b = (b + a) % W;
            Utilities.xorSymbolInPlace(result, C[b]);
        }

        while (b1 >= P)
            b1 = (b1 + a1) % P1;

        Utilities.xorSymbolInPlace(result, C[W + b1]);
    ...
    public static byte[][] PInactivationDecoding(byte[][] A, byte[][] D, int symbol_size, int Kprime)
        ...
        while (i + u != L)
        {
            ...
            for (int row = i, nonZeros = 0, degree = 0; row < M; row++, nonZeros = 0, degree = 0)
            {
                ...
                if (nonZeros == 2 && !isHDPC) rows.put(row, new Row(row, nonZeros, degree, isHDPC, edges));
            ...
            { // r = 2
                ...
                // do we have a row with exactly two 1's? (remember we already know that there are rows with only 2
                // non-zeros)
                if (minDegree == 2)
                {
                    /*
        ...
        for (int row = 0; row < i; row++)
            // multiply X by D
            D[d[row]] = Utilities.multiplyByteLineBySymbolVector(X[row], i, reorderD);
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            for (int l = 0; l < j; l++) {
                ...
                { // "then add A[j,l] multiplied with row l of A to row j of A."
                    ...
                    Utilities.xorSymbolInPlace(A[j], product);

                    // decoding process - D[d[j]] + (A[j][l] * D[d[l]])
                    product = OctectOps.betaProduct(beta, D[d[l]]);
                    Utilities.xorSymbolInPlace(D[d[j]], product);
                }
            }
        }
